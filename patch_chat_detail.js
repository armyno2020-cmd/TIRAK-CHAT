...
import { ChatMessage, ChatRoom, Language, ScreenId } from '../types';
...
interface ChatDetailScreenProps {
  ...
  onStartCall: (peerName: string, avatar: string, type: 'voice' | 'video') => void;
...
export const ChatDetailScreen: React.FC<ChatDetailScreenProps> = ({
  ...
  const handleSendVoiceNote = () => {
    if (recordingSeconds > 0) {
      onSendMessage(chatRoom.id, \`Voice note (\${recordingSeconds}s)\`, 'voice', replyingTo?.id, viewOnceEnabled);
    }
      ...
      <header className="fixed top-4 inset-x-4 md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-4xl z-50">
        <div className="bg-[#fcf8fb]/60 backdrop-blur-3xl border border-white/20 rounded-full shadow-sm flex justify-between items-center px-4 h-16 w-auto transition-all duration-300">
          <div className="flex items-center gap-3">
            <button onClick={onBack} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/20 active:scale-95 transition-all text-[#1b1b1d]">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => onStartCall(chatRoom.name, chatRoom.avatarUrl || '', 'voice')} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/20 active:scale-95 transition-all text-[#45474a]">
              <span className="material-symbols-outlined">call</span>
            </button>
            <button onClick={() => onStartCall(chatRoom.name, chatRoom.avatarUrl || '', 'video')} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/20 active:scale-95 transition-all text-[#45474a]">
          ...
          <div className="mt-2 mx-4 p-2 bg-[#ffefef]/80 backdrop-blur-md rounded-2xl border border-white/60 flex items-center justify-between text-xs shadow-sm cursor-pointer hover:bg-[#ffefef] transition-colors" onClick={() => {
            ...
            <div className="flex items-center space-x-2 overflow-hidden">
              ...
              <div className="truncate">
                ...
                <span className="text-[#45474a]">{pinnedMessage.type === 'image' ? 'Photo' : pinnedMessage.type === 'voice' ? 'Voice note' : pinnedMessage.content}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-4 relative z-10 hide-scrollbar" id="chat-container">
          <div className="flex justify-center my-8">
            <span className="font-label-caps text-[#45474a]/60 bg-[#f6f3f5]/40 px-4 py-1 rounded-full text-[10px] uppercase tracking-wider">
              {getTranslation(activeLanguage, 'chatStarted')}
            </span>
          </div>

          {messages.map((msg, index) => {
            const isMe = msg.senderId === currentUserId;
            const showAvatar = !isMe && (index === 0 || messages[index - 1].senderId === currentUserId);
                ...
                <div className={\`\${isMe ? 'bg-[#ffefef] border border-[#7e5356]/10 rounded-2xl rounded-br-none' : 'glass-surface border border-white/40 rounded-2xl rounded-bl-none'} px-5 py-3.5 shadow-[0_4px_20px_-2px_rgba(93,94,99,0.05)] relative overflow-hidden\`}>
                  ...
                  {msg.type === 'image' && (
                    <div className="relative rounded-xl overflow-hidden mb-1">
                      <img src={msg.content} alt="Attachment" className="max-w-full h-auto object-cover max-h-64" />
                    </div>
                  )}
                ...
                <div className={\`flex items-center gap-1 text-[11px] font-label-caps text-[#45474a]/50 uppercase \${isMe ? 'mr-2' : 'ml-2'}\`}>
                  ...
                  {isMe && (
                    <span className="flex items-center gap-0.5 ml-1">
                      ...
                      {msg.status === 'read' && <CheckCheck className="w-3.5 h-3.5 ml-0.5" />}
                    </span>
                  )}
                  {msg.isPinned && (
...
console.log('ChatDetailScreen updated');
